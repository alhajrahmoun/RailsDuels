module DuelHelper
  def duel_progress(correct_answers:, points:, answered_questions:)
    ...
    content_tag(:div, class: 'progress border border-dark') do
      content_tag(:div, "#{points} #{'Point'.pluralize(points)}", class: 'progress-bar bg-success', style: "width: #{correct_answers_percentage}%") +
        content_tag(:div, '', class: 'progress-bar bg-danger progress-bar-stripped', style: "width: #{wrong_answers_percentage}%") +
